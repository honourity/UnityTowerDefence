...
public class InputManager : MonoBehaviour
{
	...
	private void Update()
	{
		if (Input.GetMouseButtonDown(0))
		{
			...
			RaycastHit hit;
			if (Physics.Raycast(ray, out hit, Mathf.Infinity, GameManager.Instance.DefendersLayer))
			{
		...
		else if (Input.GetMouseButtonDown(1))
		{
			...
			Ray ray = Camera.main.ScreenPointToRay(Input.mousePosition);
			RaycastHit hit;
			if (Physics.Raycast(ray, out hit, Mathf.Infinity, GameManager.Instance.EnvironmentLayer))
			{
